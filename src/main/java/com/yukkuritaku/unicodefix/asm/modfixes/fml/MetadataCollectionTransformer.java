...
public class MetadataCollectionTransformer implements ITransformer {

    private static final Logger LOGGER = LogManager.getLogger();

    @Override
    public String[] getClassName() {
        return new String[]{TransformerClass.MetadataCollection.getTransformerName()};
    }

    @Override
    public void transform(ClassNode classNode, String name) {
        for (MethodNode node : classNode.methods){
            if (TransformerMethod.from.matches(node)){
                ...
                while (iterator.hasNext()) {
                    ...
                    if (insnNode instanceof MethodInsnNode && insnNode.getOpcode() == Opcodes.INVOKESPECIAL &&
                        ...
                        LOGGER.info("Fix mcmod.info text bugs!");
                        LOGGER.info("If you're interesting for the this bug, see: https://github.com/Yukkuritaku/unicode-fix/issues/3");
